 ...
 */
LocationChain.prototype.getOpts = function(pid, cid) {
    var _this = this,
        isCity = arguments.length === 2;

    var defaultOpt = isCity ? _this.cityDefalutOption : _this.provinceDefaultOption;
    ...
    var opts = [defaultOpt],
        dataMap = isCity ? (locationMap[pid] ? locationMap[pid].cities : {}) : locationMap;
...
for (var i = 0, len = provincesArr.length; i < len; i++) {
    var key = provincesArr[i].id;
    delete provincesArr[i]['id'];
    var value = provincesArr[i];

    var cities = {},
        cityArr = value.citys;
    for (var j = 0, l = cityArr.length; j < l; j++) {
        for (var k in cityArr[j]) {
            cities[k] = cityArr[j][k];
        }
    }
    delete value.citys;
    value['cities'] = cities;
...
function getLocation(province, city) {
    var rst = '';
    if (province) {
        $.each(locationData.provinces, function(index, elm) {
            if (elm.id == province) {
                rst = elm.name;
                if (city) {
                    rst += (',' + elm.citys[city - 1][city])
                }
                return false;
            }
        });
